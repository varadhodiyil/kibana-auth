...
const application = {
  ...
  submit: () => {
    ...
    xhr.open("POST", window.basePath + '/customlogin', true);
        ...
        if (this.readyState === XMLHttpRequest.DONE) {
            switch (this.status) {
                case 200:
                    ...
                    if (window.basePath === '') {
                      // localStorage.setItem('custom-login-username',user_name);
                      
                        document.cookie = "custom-login-username="+user_name;

                        // location.href = '/';
                        location.href = '/app/kibana#/dashboard';
                    } else {
                      // localStorage.setItem('custom-login-username',user_name);
                      document.cookie = "custom-login-username="+user_name;
                      location.href = window.basePath+ '/app/kibana#/dashboard';
                    }
                    break;
                case 406:
